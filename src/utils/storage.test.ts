...
describe('loadSets', () => {
  ...
  it('returns [] on corrupt JSON', () => {
    localStorage.setItem('mgrs-gpx-sets', 'not json')
    expect(loadSets()).toEqual([])
  })
  ...
  it('migrates legacy waypoints field to markers', () => {
    ...
    localStorage.setItem('mgrs-gpx-sets', JSON.stringify([legacy]))
  ...
  it('prefers markers over waypoints when both are present', () => {
    ...
    localStorage.setItem('mgrs-gpx-sets', JSON.stringify([both]))
  ...
  it('falls back to [] when both markers and waypoints are absent', () => {
    ...
    localStorage.setItem('mgrs-gpx-sets', JSON.stringify([empty]))
